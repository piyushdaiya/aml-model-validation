...
function ReportPreviewView() {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[280px_1fr_320px]">
        <SectionCard title="Outline" description="Structured report sections aligned to validation methodology." contentClassName="p-4">
          {report ? (
            <ReportOutline sections={report.sections} activeSectionId={activeSectionId} onSelect={setActiveSectionId} />
          ) : null}
        </SectionCard>

        <SectionCard title={report?.title ?? "Report not found"} description={model?.summary ?? "No report is available for this model."} contentClassName="space-y-6 p-6">
          {activeSection ? (
            <>
...
export default function ReportPreviewPage() {
  return (
    <DemoShell
      ...
      description="Consulting-grade report packaging that shows leadership and client teams how the accelerator becomes a regulator- and audit-friendly product surface."
      actions={
