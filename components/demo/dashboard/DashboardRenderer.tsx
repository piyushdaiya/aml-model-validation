...
function getMetricCards(data: DashboardData): Record<DashboardKpiId, MetricCardConfig> {
  ...
  const clientOverdueFindings = clientScope.findings.filter((finding) => finding.status !== "Closed" && isOverdue(finding.dueDate))
  const clientNearReview = clientScope.models.filter((model) => daysUntil(model.nextReadoutDate) <= 21)
  const clientAuditExceptions =
  ...
  const clientReadyReports = clientScope.reports.filter(
    (report) => report.sections.every((section) => section.status !== "Drafted")
  ).length
  const clientDriftIssues = clientScope.dataChecks.filter((check) => check.stream === "Drift" && check.status !== "Pass").length
  const clientFailedScenarios = clientScope.scenarios.filter((scenario) => scenario.status === "Fail").length
  const clientThresholdExceptions = clientScope.scenarios.filter(
    (scenario) =>
      ["Sensitivity", "Stress", "Adversarial", "Below-the-Line"].includes(scenario.kind) && scenario.status !== "Pass"
  ).length
  const clientOpenRemediations = clientScope.findings.filter((finding) => finding.status !== "Closed").length
  const clientBenchmarkGaps = clientScope.models.filter(
  ...
  const assignedActions = clientScope.findings.filter((finding) => finding.status !== "Closed").length
  ...
  const trainingTasks = clientScope.models.filter(
    (model) => model.stage === "Planning" || model.stage === "Testing" || model.status !== "Completed"
  ).length
  const failedTestRuns =
    clientScope.scenarios.filter((scenario) => scenario.status === "Fail").length +
    clientScope.dataChecks.filter((check) => check.status === "Fail").length
  const dataQualityExceptions = clientScope.dataChecks.filter((check) => check.status !== "Pass").length
  const externalDataAlerts = clientScope.dataChecks.filter(
    (check) => (check.stream === "Integrity" || check.stream === "Lineage") && check.status !== "Pass"
  ).length
  const calibrationChanges = clientScope.activity.filter((item) => item.type === "calibration-changed").length
  ...
  return {
    HighRiskFindingsKpi: {
      ...
      subtitle: "Critical and high issues requiring immediate compliance attention.",
      accent: "rose",
      icon: ShieldAlert,
    },
    OverdueRemediationsKpi: {
      title: "Overdue Remediations",
      value: `${clientOverdueFindings.length}`,
      subtitle: "Open items already past due against the agreed remediation calendar.",
      accent: "amber",
      icon: AlertTriangle,
    },
    RevalidationTriggersKpi: {
      ...
      subtitle: "Triggers opened from drift, slippage, or governance events.",
      accent: "blue",
      icon: RotateCcw,
    },
    EvidencePacksReadyKpi: {
      title: "Evidence Packs Ready",
      value: `${clientReadyReports}`,
      subtitle: "Client packs with drafted sections cleared for leadership-facing review.",
      accent: "emerald",
      icon: ScrollText,
    },
    ModelsNearReviewDateKpi: {
      title: "Models Near Review Date",
      value: `${clientNearReview.length}`,
      subtitle: "Models approaching steering committee or committee readout dates in the next three weeks.",
      accent: "slate",
      icon: CalendarClock,
    },
    AuditTrailExceptionsKpi: {
      title: "Audit Trail Exceptions",
      value: `${clientAuditExceptions}`,
      subtitle: "Lineage, integrity, or reporting-control gaps that would matter in an audit replay.",
      accent: "amber",
      icon: FileText,
    },
    MaterialRiskIssuesKpi: {
      ...
      subtitle: "Combined view of severe findings and escalated models.",
      accent: "rose",
      icon: ShieldAlert,
    },
    PerformanceDriftKpi: {
      title: "Performance Drift",
      value: `${clientDriftIssues}`,
      subtitle: "Active drift exceptions from the current data-validation and testing package.",
      accent: "amber",
      icon: TrendingUp,
    },
    FailedScenarioKpi: {
      title: "Failed Scenarios",
      value: `${clientFailedScenarios}`,
      subtitle: "Scenarios that currently fail the validation threshold for passable evidence.",
      accent: "rose",
      icon: FlaskConical,
    },
    ThresholdExceptionsKpi: {
      title: "Threshold Exceptions",
      value: `${clientThresholdExceptions}`,
      subtitle: "Sensitivity, stress, and adversarial scenarios still outside target behavior.",
      accent: "blue",
      icon: SlidersHorizontal,
    },
    OpenRemediationsKpi: {
      title: "Open Remediations",
      value: `${clientOpenRemediations}`,
    ...
    BenchmarkGapsKpi: {
      ...
      value: `${clientBenchmarkGaps}`,
      subtitle: "Models falling below target precision or ROC-AUC benchmarks.",
      accent: "amber",
    ...
    AssignedModelsKpi: {
      ...
      subtitle: "Models currently in scope for owner documentation and response activity.",
      accent: "blue",
      icon: FolderKanban,
    },
    PendingDocumentationKpi: {
      ...
      subtitle: "Draft or under-review report sections and owner-ready documentation items.",
      accent: "amber",
      icon: FileText,
    },
    AssignedActionsKpi: {
      title: "Assigned Actions",
      value: `${assignedActions}`,
    ...
    ValidationResponseItemsKpi: {
      ...
      subtitle: "Issues currently expecting model-owner response or governance narrative.",
      accent: "slate",
    ...
    TrainingTasksKpi: {
      ...
      value: `${trainingTasks}`,
      subtitle: "Owner enablement and prep work implied by models still in planning or active testing.",
      accent: "blue",
      icon: BookText,
    },
    ...
    ExternalDataAlertsKpi: {
      ...
      value: `${externalDataAlerts}`,
      subtitle: "Source-integrity and lineage alerts that will affect downstream validation reliability.",
      accent: "amber",
      icon: Globe2,
    },
    CalibrationChangesKpi: {
      title: "Calibration Changes",
      value: `${calibrationChanges}`,
      subtitle: "Recorded threshold or calibration changes across recent test execution.",
      accent: "blue",
      icon: SlidersHorizontal,
    },
    EvidenceCompletenessKpi: {
      ...
      subtitle: "Fresh evidence artifacts available for the active findings and test scenarios.",
      accent: "emerald",
      icon: CheckCircle2,
    },
    ActiveClientEngagementsKpi: {
      ...
      subtitle: "Portfolio-wide client engagements represented in the consulting accelerator demo.",
      accent: "blue",
    ...
    ModelsInScopeKpi: {
      ...
      subtitle: "Total AML models available across the current multi-client workbench.",
      accent: "slate",
      icon: FolderKanban,
    },
    ActiveUsersKpi: {
      ...
      subtitle: "Distinct mock stakeholders reflected across sponsors, owners, validators, and administrators.",
      accent: "emerald",
      icon: Users,
    },
    DemoHealthKpi: {
    ...
    ConfigChangesKpi: {
      ...
      value: `${configChanges}`,
      subtitle: "Recent configuration changes captured in the admin activity stream.",
      accent: "blue",
      icon: Settings2,
    },
  }
...
function ListBlock({
  ...
  if (items.length === 0) {
    return <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">{emptyMessage}</div>
  }
...
function WidgetRenderer({
  ...
  const clientSeverity = getFindingSeverityCounts(data.clientScope.findings)
  const severityData = [
    { label: "Low", count: clientSeverity.Low },
    { label: "Moderate", count: clientSeverity.Moderate },
    { label: "High", count: clientSeverity.High },
    { label: "Critical", count: clientSeverity.Critical },
  ]

  const validationStatusSeries = data.clientScope.models.map((model) => ({
  ...
  const performanceSeries = data.clientScope.models.map((model) => ({
    name: model.name.split(" ").slice(0, 2).join(" "),
  ...
  switch (widgetId) {
    case "FindingsSeverityWidget":
      return (
        <SectionCard
          ...
          description="Severity mix for the active client engagement."
              ...
              <div key={item.label}>
                ...
                <div className="h-2 rounded-full bg-slate-100">
                  <div
                    ...
                    style={{ width: `${Math.max(8, item.count * 28)}px` }}
                  />
                </div>
              </div>
            ))}
    ...
    case "RemediationTimelineWidget":
      return (
        <SectionCard
          ...
          description="Due-date pressure for open findings and management actions."
    ...
    case "RevalidationTriggerWatchlist":
      return (
        <SectionCard
          ...
          description="Models and events currently signaling the need for accelerated retest or committee escalation."
          contentClassName="p-6"
        >
          <ListBlock
            items={[
    ...
    case "ValidationStatusWidget":
      return (
        <SectionCard
          ...
          description="Progress, ATL coverage, and BTL coverage across the active client’s models."
          contentClassName="p-6"
        >
    ...
    case "EvidencePackReadinessWidget":
      return (
        <SectionCard
          ...
          description="Report pack readiness across sections tied to the active client’s models."
          contentClassName="p-6"
        >
          <ListBlock
    ...
    case "RiskHeatmapWidget":
      return (
        <SectionCard
          title="Risk Heatmap"
          description="Severity and likelihood concentration for the active engagement."
          contentClassName="p-6"
        >
    ...
    case "PerformanceTrendWidget":
      return (
        <SectionCard
          ...
          description="Precision, recall, and ROC-AUC viewed across the current model set."
          contentClassName="p-6"
        >
          <div className="h-[250px]">
            {chartsReady ? (
    ...
    case "ScenarioFailureWidget":
      return (
        <SectionCard
          ...
          description="Most material failed or watch scenarios from the active test program."
          contentClassName="p-6"
        >
          <ListBlock
    ...
    case "ThresholdSensitivityWidget":
      return (
        <SectionCard
          ...
          description="Current calibration controls from the active model workspace."
          contentClassName="p-6"
        >
          <ListBlock
            ...
            emptyMessage="No threshold controls are available for the active model."
          />
        </SectionCard>
      )
    case "RevalidationMilestonesWidget":
      return (
        <SectionCard
          title="Revalidation Milestones"
          description="Upcoming milestone checkpoints for near-term review dates."
          contentClassName="p-6"
        >
          <AuditTimeline
            compact
            items={data.clientScope.models
              ...
              .sort((left, right) => parseDate(left.meta).getTime() - parseDate(right.meta).getTime())
              .slice(0, 3)}
          />
        </SectionCard>
      )
    case "AssignedModelsWidget":
      return (
        <SectionCard
          title="Assigned Models"
          description="Model scope, stage, and risk status for the current owner lens."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.clientScope.models.map((model) => ({
              id: model.id,
              title: model.name,
              detail: `${model.type} • owner ${model.owner}`,
              meta: `${model.stage} • readout ${model.nextReadoutDate}`,
              chip: model.status,
              chipKind: "milestone" as const,
            }))}
            emptyMessage="No models are assigned in the current scope."
          />
        </SectionCard>
      )
    case "DocumentationChecklistWidget":
      return (
        <SectionCard
          ...
          description="Sections and artifacts still needing owner completion."
          contentClassName="p-6"
        >
          <ListBlock
            ...
            emptyMessage="All report sections for the selected client are already marked ready."
          />
        </SectionCard>
      )
    case "MilestonesWidget":
      return (
        <SectionCard
          title="Milestones"
          description="Active-model timeline for governance preparation."
          contentClassName="p-6"
        >
          <AuditTimeline
            compact
            items={data.activeModelMilestones.map((milestone) => ({
              id: `${data.activeModelId}-${milestone.label}`,
              title: milestone.label,
              subtitle: data.activeModelName,
              meta: milestone.dueDate,
              status: milestoneToTimelineStatus(milestone.status),
            }))}
          />
    ...
    case "OwnerResponseFindingsWidget":
      return (
        <SectionCard
          title="Owner Response Findings"
          description="Open issues where owner narrative or closure support is still expected."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.clientScope.findings
            ...
            emptyMessage="No owner-response items are currently open."
          />
        </SectionCard>
      )
    case "NotesCommentsWidget":
      return (
        <SectionCard
          ...
          description="Recent governance comments, note additions, and documentation requests."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.clientScope.activity
              .filter((item) =>
                ["governance-comment-added", "document-requested", "audit-note-added"].includes(item.type)
              )
              .slice(0, 4)
              .map((item) => ({
                id: item.id,
            ...
            emptyMessage="No recent owner-facing comments or notes were found."
          />
        </SectionCard>
      )
    case "TestingQueueWidget":
      return (
        <SectionCard
          title="Testing Queue"
          description="Current scenario execution queue for the selected client."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.clientScope.scenarios.slice(0, 4).map((scenario) => ({
              ...
              meta: `${scenario.kind} • reproducibility ${scenario.reproducibilityScore}%`,
              chip: scenario.status,
              chipKind: "risk" as const,
            }))}
            emptyMessage="No testing scenarios are queued for the current scope."
          />
        </SectionCard>
      )
    case "DataQualityExceptionsWidget":
      return (
        <SectionCard
          title="Data-Quality Exceptions"
          description="Current watch and fail conditions from the active validation workspaces."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.clientScope.dataChecks
              ...
              .map((check) => ({
                ...
                title: check.name,
                detail: check.result,
                meta: `${check.stream} • owner ${check.owner}`,
                chip: check.status,
                chipKind: "risk" as const,
              }))}
            emptyMessage="No data-quality exceptions are currently open."
          />
        </SectionCard>
      )
    case "EvidenceCompletenessWidget":
      return (
        <SectionCard
          title="Evidence Completeness"
          description="Evidence artifacts linked to the active model’s findings and tests."
          contentClassName="p-6"
        >
          <ListBlock
            items={getEvidenceForIds(
              Array.from(
                new Set([
                  ...data.clientScope.findings
                    .filter((finding) => finding.modelId === data.activeModelId)
                    .flatMap((finding) => finding.evidenceIds),
                  ...(getTestingWorkspace(data.activeModelId)?.scenarios.flatMap((scenario) => scenario.evidenceIds) ?? []),
                ])
              )
            ).map((item) => ({
              id: item.id,
              title: item.title,
              detail: item.note,
              meta: `${item.category} • updated ${item.updatedAt}`,
              chip: daysUntil(item.updatedAt) >= -3 ? "Current" : "Refresh",
              chipKind: "milestone" as const,
            }))}
            emptyMessage="No evidence items are mapped to the active model yet."
          />
        </SectionCard>
      )
    case "ScenarioCoverageWidget":
      return (
        <SectionCard
          ...
          description="Coverage by testing scenario type across the selected client."
          contentClassName="p-6"
        >
          <div className="h-[250px]">
            {chartsReady ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={scenarioCoverage}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                  <XAxis dataKey="kind" stroke="#64748b" fontSize={12} />
    ...
    case "ExternalSourceIntegrityWidget":
      return (
        <SectionCard
          title="External Source Integrity"
          description="Integrity and lineage checks tied to upstream or external data dependencies."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.clientScope.dataChecks
            ...
            emptyMessage="No source-integrity checks are currently configured."
          />
        </SectionCard>
      )
    case "CalibrationHistoryWidget":
      return (
        <SectionCard
          ...
          description="Recent calibration changes and active threshold control state."
          contentClassName="p-6"
        >
          <ListBlock
            items={[
              ...data.clientScope.activity
                .filter((item) => item.type === "calibration-changed")
                .slice(0, 2)
                .map((item) => ({
                  id: item.id,
                  title: item.title,
              ...
              ...(getTestingWorkspace(data.activeModelId)?.thresholdControls.slice(0, 2).map((control) => ({
                id: control.label,
                title: control.label,
                detail: `Current value ${control.value}${control.unit} with bounds ${control.min}-${control.max}${control.unit}.`,
                meta: data.activeModelName,
              })) ?? []),
            ]}
            emptyMessage="No calibration history is available for the current scope."
          />
        </SectionCard>
      )
    case "ClientPortfolioWidget":
      return (
        <SectionCard
          ...
          description="Portfolio-wide engagements in the accelerator demo."
          contentClassName="p-6"
        >
          <ListBlock
            items={clientEngagements.map((engagement) => ({
              ...
              meta: `${engagement.engagementName} • committee ${engagement.nextCommitteeDate}`,
              chip: engagement.stage,
              chipKind: "milestone" as const,
            }))}
            emptyMessage="No client engagements are configured."
          />
        </SectionCard>
      )
    case "ModelStatusOverviewWidget":
      return (
        <SectionCard
          ...
          description="Portfolio counts by model status across all demo engagements."
          contentClassName="space-y-4 p-6"
        >
          {Object.entries(getModelCountsByStatus(data.portfolioScope.models)).map(([status, count]) => (
            <div key={status}>
              <div className="mb-2 flex items-center justify-between text-sm">
              ...
              <div className="h-2 rounded-full bg-slate-100">
                <div className="h-2 rounded-full bg-slate-900" style={{ width: `${Math.max(10, count * 26)}px` }} />
              </div>
            </div>
          ))}
        </SectionCard>
    ...
    case "DemoSystemActivityWidget":
      return (
        <SectionCard
          title="Demo System Activity"
          description="Portfolio-level admin and operational events captured in the mock activity stream."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.portfolioScope.activity
              .filter((item) =>
                ["persona-changed", "client-switched", "configuration-updated", "demo-health-warning"].includes(item.type)
              )
              .slice(0, 4)
              .map((item) => ({
                id: item.id,
            ...
            emptyMessage="No system-activity items are currently recorded."
          />
        </SectionCard>
      )
    case "ConfigurationLogWidget":
      return (
        <SectionCard
          ...
          description="Recent admin changes affecting demo composition and walkthrough setup."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.portfolioScope.activity
    ...
    case "SupportQueueWidget":
      return (
        <SectionCard
          title="Support Queue"
          description="Admin-facing support and operational items from the current demo environment."
          contentClassName="p-6"
        >
          <ListBlock
            items={data.portfolioScope.activity
    ...
    case "AccessOverviewWidget":
      return (
        <SectionCard
          ...
          description="Portfolio context for roles, selected client, and visible operating scope."
          contentClassName="space-y-4 p-6"
        >
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
              ...
              <p className="mt-2 text-sm font-medium text-slate-900">{data.clientScope.models.length} models</p>
            </div>
          </div>
        </SectionCard>
      )
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {config.ctas.map((cta, index) => (
        <Button key={cta.label} variant={index === 0 ? "default" : "outline"} asChild className={index === 0 ? "bg-slate-950 text-white hover:bg-slate-800" : ""}>
  ...
  const data = useMemo<DashboardData>(() => {
    ...
    return {
      ...
      selectedActivity: getSelectedActivity(
        personaId === "admin" ? portfolioActivity : clientActivity,
